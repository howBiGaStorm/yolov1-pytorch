...
class VOCdetection(data.Dataset):
    ...
    def __init__(self,root,target_transform = VOCAnnotationTransform()):
        ...
        with open(VOC_2007t_ROOT+'ImageSets/Main/'+'evall.txt') as f:
            all_id = f.readlines()
        for id in range(len(all_id)):
            self.ids.append((VOC_2007t_ROOT,all_id[id].strip()))

    def __getitem__(self, item):
        img, gt, height, width,path_and_id = self.pull_item(item)
        return img, gt,height,width,path_and_id
